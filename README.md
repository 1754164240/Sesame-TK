...
| 日期 | 模块 | 功能改动 |
| --- | --- | --- |
| 2026-07-29 | RPC 安全 / 持久调度 / 设置中心 | RPC 失败统一区分离线、验证、可重试与不可重试状态，安全验证触发后暂停自动任务；新增按账号隔离的持久调度、系统闹钟、Binder 跨进程路由、重启恢复和协程回退基础，首页按当前账户显示持久拉起“已开启、已关闭、未确认”三态；Web 设置页新增时间点与时间窗口编辑、`-1` 禁用、列表搜索与标签筛选、批量选择、计数同步、默认值恢复和好友范围只读预览。 |
