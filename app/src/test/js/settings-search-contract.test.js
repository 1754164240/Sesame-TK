...
test('搜索期间隐藏展开模块且清空后恢复原展开键', () => {
    const activeKey = 'AntForest';
    const searched = filterSettingsModels(tabs, modelsMap, 'AI摸鱼');
    const cleared = filterSettingsModels(tabs, modelsMap, '');

    assert.equal(visibleSettingsActiveKey(searched, activeKey), undefined);
    assert.equal(visibleSettingsActiveKey(cleared, activeKey), 'AntForest');
});
